...
public class ProtoCommon {
	 ...
	 */
	public static RecvHeaderInfo recvHeader(byte[] in, byte expect_cmd, long expect_body_len) throws IOException {
		byte[] header;
		long pkg_len;
	 ...
	 */
	public static RecvPackageInfo recvPackage(byte[] in, byte expect_cmd, long expect_body_len, RecvPackageInfo packageInfo) throws IOException {
		RecvPackageInfo recv;
		if (packageInfo == null) {
			...
			recv = new RecvPackageInfo((byte) -1, body);
			recv.setSrcPos(FDFS_PROTO_PKG_LEN_SIZE + 2);
			recv.setDestPos(0);
		} else {
			recv = packageInfo;
		}
		System.arraycopy(in, recv.getSrcPos(), recv.body, recv.getDestPos(), in.length - recv.getSrcPos());
		recv.setDestPos(recv.getDestPos() + in.length - recv.getSrcPos());
		if (recv.getDestPos() == recv.body.length) {
			recv.errno = (byte) 0;
		} else {
			recv.setSrcPos(0);
		}
