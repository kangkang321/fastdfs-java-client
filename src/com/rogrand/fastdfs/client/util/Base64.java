...
/**
 * Encode arbitrary binary into printable ASCII using BASE64 encoding. very loosely based on the Base64 Reader by Dr. Mark Thornton Optrak Distribution Software
 * Ltd. http://www.optrak.co.uk and Kevin Kelley's http://www.ruralnet.net/~kelley/java/Base64.java Base64 is a way of encoding 8-bit characters using only
 * ASCII printable characters similar to UUENCODE. UUENCODE includes a filename where BASE64 does not. The spec is described in RFC 2045. Base64 is a scheme
 * where 3 bytes are concatenated, then split to form 4 groups of 6-bits each; and each 6-bits gets translated to an encoded printable ASCII character, via a
 * table lookup. An encoded string is therefore longer than the original by about 1/3. The "=" character is used to pad the end. Base64 is used, among other
 * things, to encode the user:password string in an Authorization: header for HTTP. Don't confuse Base64 with x-www-form-urlencoded which is handled by
 * Java.net.URLEncoder.encode/decode If you don't like this code, there is another implementation at http://www.ruffboy.com/download.htm Sun has an undocumented
 * method called sun.misc.Base64Encoder.encode. You could use hex, simpler to code, but not as compact. If you wanted to encode a giant file, you could do it in
 * large chunks that are even multiples of 3 bytes, except for the last chunk, and append the outputs. To encode a string, rather than binary data
 * java.net.URLEncoder may be better. See printable characters in the Java glossary for a discussion of the differences. version 1.4 2002 February 15 -- correct
 * bugs with uneven line lengths, allow you to configure line separator. now need Base64 object and instance methods. new mailing address. version 1.3 2000
 * September 12 -- fix problems with estimating output length in encode version 1.2 2000 September 09 -- now handles decode as well. version 1.1 1999 December
 * 04 -- more symmetrical encoding algorithm. more accurate StringBuffer allocation size. version 1.0 1999 December 03 -- posted in comp.lang.java.programmer.
...
public class Base64 {
	...
	/**
	 * Encode an arbitrary array of bytes as Base64 printable ASCII. It will be broken into lines of 72 chars each. The last line is not terminated with a line
	 * separator. The output will always have an even multiple of data characters, exclusive of \n. It is padded out with =.
	 */
	...
	public static void main(String[] args) {
		...
		if (debug) {
			...
			/*
			 * byte[] a = { (byte)0xfc, (byte)0x0f, (byte)0xc0}; byte[] b = { (byte)0x03, (byte)0xf0, (byte)0x3f}; byte[] c = { (byte)0x00, (byte)0x00,
			 * (byte)0x00}; byte[] d = { (byte)0xff, (byte)0xff, (byte)0xff}; byte[] e = { (byte)0xfc, (byte)0x0f, (byte)0xc0, (byte)1}; byte[] f = {
			 * (byte)0xfc, (byte)0x0f, (byte)0xc0, (byte)1, (byte)2}; byte[] g = { (byte)0xfc, (byte)0x0f, (byte)0xc0, (byte)1, (byte)2, (byte)3}; byte[] h =
			 * "AAAAAAAAAAB".getBytes(); show(a); show(b); show(c); show(d); show(e); show(f); show(g); show(h); Base64 b64 = new Base64();
			 * show(b64.decode(b64.encode(a))); show(b64.decode(b64.encode(b))); show(b64.decode(b64.encode(c))); show(b64.decode(b64.encode(d)));
			 * show(b64.decode(b64.encode(e))); show(b64.decode(b64.encode(f))); show(b64.decode(b64.encode(g))); show(b64.decode(b64.encode(h)));
